...
//---------------------------------------------------------------------------
bool ConfigStoreFile::GetWholeSect(EasyStringList *pESL,EasyStr Sect,bool Upr)
{
  strupr(Sect);
  int iSect=Sects.NumItems-1;
  for (;iSect>=0;iSect--){
    if (strcmp(Sects[iSect].szNameUpr,Sect)==0) break;
  }
  if (iSect<0) return 0;
...
//---------------------------------------------------------------------------
void ConfigStoreFile::DeleteSection(EasyStr Sect)
{
  int s;
  strupr(Sect);
  for (s=Sects.NumItems-1;s>=0;s--){
    if (strcmp(Sects[s].szNameUpr,Sect)==0) break;
  }
  if (s<0) return;
  for (int k=0;k<Keys.NumItems;k++){
    if (Keys[k].iSect==s) Keys.Delete(k--);
  }
  Changed=true;
}
//---------------------------------------------------------------------------
