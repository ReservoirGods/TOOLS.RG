...
#define m68k_READ_B_FROM_ADDR                         \
  ...
  if(abus>=himem){                                  \
    if(abus>=MEM_IO_BASE){            \
      ...
      if(tos_high && abus<(0xfc0000+tos_len))m68k_src_b=ROM_PEEK(abus-rom_addr);   \
      ...
      if(abus<(0xe00000+tos_len))m68k_src_b=ROM_PEEK(abus-rom_addr);                           \
...
#define m68k_READ_W_FROM_ADDR           \
  ...
  if(abus&1){                                      \
    ...
    if(abus>=MEM_IO_BASE){            \
      ...
      if (tos_high && abus<(0xfc0000+tos_len)) m68k_src_w=ROM_DPEEK(abus-rom_addr);   \
      else if (abus<0xfe0000 || abus>=0xfe2000) exception(BOMBS_BUS_ERROR,EA_READ,abus);  \
    }else if(abus>=MEM_EXPANSION_CARTRIDGE){           \
      if(cart){                                             \
      ...
      if(abus<(0xe00000+tos_len)) m68k_src_w=ROM_DPEEK(abus-rom_addr);                           \
      else if (abus>=0xec0000) exception(BOMBS_BUS_ERROR,EA_READ,abus);          \
...
#define m68k_READ_L_FROM_ADDR                        \
  ...
  if(abus&1){                                      \
    ...
    if(abus>=MEM_IO_BASE){           \
      ...
      if(tos_high && abus<(0xfc0000+tos_len-2)) m68k_src_l=ROM_LPEEK(abus-rom_addr);   \
      else if (abus<0xfe0000 || abus>=0xfe2000) exception(BOMBS_BUS_ERROR,EA_READ,abus);  \
    }else if(abus>=MEM_EXPANSION_CARTRIDGE){           \
      if(cart){                                             \
      ...
      if(abus<(0xe00000+tos_len-2)) m68k_src_l=ROM_LPEEK(abus-rom_addr);   \
      else if (abus>=0xec0000) exception(BOMBS_BUS_ERROR,EA_READ,abus);          \
...
BYTE m68k_peek(MEM_ADDRESS ad){
  ...
  if (ad>=himem){
    if (ad>=MEM_IO_BASE){
      ...
      if(tos_high && ad<(0xfc0000+tos_len))return ROM_PEEK(ad-rom_addr);
      ...
      if (ad<(0xe00000+tos_len)) return ROM_PEEK(ad-rom_addr);
...
WORD m68k_dpeek(MEM_ADDRESS ad){
  ...
  if(ad&1)exception(BOMBS_ADDRESS_ERROR,EA_READ,ad);
    ...
    if(ad>=MEM_IO_BASE){
      ...
      if(tos_high && ad<(0xfc0000+tos_len))return ROM_DPEEK(ad-rom_addr);
      else if (ad<0xfe0000 || ad>=0xfe2000) exception(BOMBS_BUS_ERROR,EA_READ,ad);
    }else if(ad>=MEM_EXPANSION_CARTRIDGE){
      if (cart) return CART_DPEEK(ad-MEM_EXPANSION_CARTRIDGE);
      else return 0xffff;
    }else if(ad>=rom_addr){
      if (ad<(0xe00000+tos_len)) return ROM_DPEEK(ad-rom_addr);
      if (ad>=0xec0000) exception(BOMBS_BUS_ERROR,EA_READ,ad);
      return 0xffff;
    }else if (ad>=0xd00000 && ad<0xd80000){
      return 0xffff;
...
LONG m68k_lpeek(MEM_ADDRESS ad){
  ...
  if(ad&1)exception(BOMBS_ADDRESS_ERROR,EA_WRITE,ad);
    ...
    if(ad>=MEM_IO_BASE){
      ...
      if(tos_high && ad<(0xfc0000+tos_len-2))return ROM_LPEEK(ad-rom_addr);
      else if (ad<0xfe0000 || ad>=0xfe2000) exception(BOMBS_BUS_ERROR,EA_READ,ad);
    }else if (ad>=MEM_EXPANSION_CARTRIDGE){
      if (cart) return CART_LPEEK(ad-MEM_EXPANSION_CARTRIDGE);
      else return 0xffffffff;
    }else if (ad>=rom_addr){
      if (ad<(0xe00000+tos_len-2)) return ROM_LPEEK(ad-rom_addr);
      if (ad>=0xec0000) exception(BOMBS_BUS_ERROR,EA_READ,ad);
      return 0xffffffff;
    }else if (ad>=0xd00000 && ad<0xd80000){
      return 0xffffffff;
