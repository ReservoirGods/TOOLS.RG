...
DEBUG_ADDRESS* debug_find_or_add_address(MEM_ADDRESS ad)
{
  ...
  if (pda==NULL){
	  DEBUG_ADDRESS da={ad,1,0,{0,0},{0},{0}};
   da.BreakCondition[0]='1';
   da.BreakCondition[1]=0;
   ExpressionEvaluator::EE_Expression_68K_Compile( da.BreakCondition, &da.BreakExpression );
    debug_ads.Add(da);
  }
...
//---------------------------------------------------------------------------
void debug_set_breakcondition(MEM_ADDRESS ad,EasyStr name)
{
	DEBUG_ADDRESS *pda=debug_find_or_add_address(ad);


	strcpy(pda->BreakCondition,name.Lefts(127));

	ExpressionEvaluator::EE_Expression_68K_Compile( pda->BreakCondition, &pda->BreakExpression );
...
int debug_evaluate_breakcondition( int aAddress )
{
	DEBUG_ADDRESS *pda=debug_find_address(aAddress);
	if (pda )
	{
		if( 0 != ExpressionEvaluator::EE_Expression_Evaluate( &pda->BreakExpression ) )
		{
			return( 1 );
		}
	}
      ...
      if (HIWORD(wPar)==STN_CLICKED || HIWORD(wPar)==BN_CLICKED){
        if (id>=4000 && id<=4150){
          ...
          SetForegroundWindow(m_b[id-950]->owner);
			...
			extern U32 gProfilerWindowSymbolAddress;
			new mem_browser( gProfilerWindowSymbolAddress, DT_INSTRUCTION );
          ...
          switch (action){
			...
			case 20:
				EnableAllWindows(0,mb->owner);
				Str NewCondition=pda->BreakCondition;
				if (InputPrompt_Choose(mb->owner,"Breakpoint Condition",NewCondition)){
					debug_set_breakcondition(pda->ad,NewCondition);
				}
				EnableAllWindows(true,mb->owner);
				break;
          }
...
*/
    ...
    case WM_INITMENUPOPUP:
      if ((HMENU)wPar==mem_browser_menu){
        ...
        breakpoint_menu_setup();
        return 0;
	  }else if ((HMENU)wPar==profiler_menu){
      }
      break;
    case WM_DRAWITEM:
    {
...
void DWin_init()
{
  ...
  AppendMenu(logsection_menu,MF_STRING,1012,"Suspend Logging");
	...
	profiler_menu = CreatePopupMenu();
	AppendMenu(menu,MF_STRING | MF_POPUP,(UINT)profiler_menu,"&Profiler");

	AppendMenu(profiler_menu,MF_STRING,1202,"Profile Reset");
	AppendMenu(profiler_menu,MF_STRING,1201,"Profile Dump To Log");
//	InsertMenu(profiler_menu,0,MF_BYPOSITION | MF_STRING,2201,"Profile Window");
	AppendMenu(profiler_menu,MF_STRING,2201,"Profile Window");
