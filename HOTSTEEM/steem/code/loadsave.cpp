...
void LoadState(GoodConfigStoreFile *pCSF)
{
  ...
  for (int n=0;;n++){
	...
	strcpy(da.BreakCondition,pCSF->GetStr("Debug Addresses",Str("BreakCondition")+n,"1"));
...
void SaveState(ConfigStoreFile *pCSF)
{
  ...
  for (int n=0;n<debug_ads.NumItems;n++){
	...
	pCSF->SetStr("Debug Addresses",Str("BreakCondition")+n,debug_ads[n].BreakCondition);
  }
