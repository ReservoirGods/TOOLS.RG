...
void mem_browser::new_window(MEM_ADDRESS address,type_disp_type new_disp_type)
{
  ...
  if (n>=MAX_MEMORY_BROWSERS){
    ...
    owner=CreateWindowEx(ex_style,"Steem Mem Browser Window",Title,
        ...
        10,20,800,600,NULL,NULL,Inst,0);
...
void mem_browser::init()
{
  ...
  if (disp_type==DT_INSTRUCTION){
	...
	lc.cx=200;
	lc.pszText="Symbol";
	cycles_column=columns;
	SendMessage(handle,LVM_INSERTCOLUMN,columns++,(long)&lc);

	lc.cx=100;
	lc.pszText="Total Cycles";
	cycles_column=columns;
	SendMessage(handle,LVM_INSERTCOLUMN,columns++,(long)&lc);

	lc.cx=100;
	lc.pszText="Frame Cycles";
	cycles_column=columns;
	SendMessage(handle,LVM_INSERTCOLUMN,columns++,(long)&lc);
...
//    if(mode==MB_MODE_PC)ad=pc; //**** CHANGED!!
  ...
  //      ldpc2=ad+i*2*wpl;
        ...
        hex_map.Add(line_hex_map);
		strcat(txt[6], Profiler::GetSymbolName( ldpc2) );
		sprintf( txt[7], "%ld", Profiler::GetInstructionCycleCountTotal( ldpc2 ) );
		sprintf( txt[8], "%ld", Profiler::GetInstructionCycleCountFrame( ldpc2 ) );
  ...
  if (col<=mon_column){
    ...
    for (int w=0;w<wpl;w++){
      ...
      if (is_bk){
		...
		AppendMenu(insp_menu,MF_BYCOMMAND,id_base+20,Str("Edit breakpoint at $")+HEXSl(row_ad,6));
      }else{
