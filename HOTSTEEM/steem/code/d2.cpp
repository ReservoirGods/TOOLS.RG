...
WORD d2_dpeek(MEM_ADDRESS ad)
{
  ...
  if(ad>=mem_len){
    ...
    if(ad>=MEM_IO_BASE){
      ...
      if(ad>=0xfc0000){
        if(tos_high && ad<(0xfc0000+tos_len))return (WORD)ROM_DPEEK(ad-rom_addr);
        else {d2_peekvalid=0xffff;return 0;}
      }else if(cart){
        return CART_DPEEK(ad-MEM_EXPANSION_CARTRIDGE);
      }
    }else if(ad>=rom_addr){
      if(ad<=(0xe00000+tos_len))      return (WORD)ROM_DPEEK(ad-rom_addr);
      else {d2_peekvalid=0xffff;return 0;}
    }else if(mmu_confused){
...
//        }
      ...
      if(ad>=0xfc0000){
        if(tos_high && ad<(0xfc0000+tos_len))return (LONG)ROM_LPEEK(ad-rom_addr);
        else {d2_peekvalid=0xffffffff;return 0;}
      }else if(cart){
        return CART_LPEEK(ad-MEM_EXPANSION_CARTRIDGE);
      }
    }else if(ad>=rom_addr){
      if(ad<=(0xe00000+tos_len))      return (LONG)ROM_LPEEK(ad-rom_addr);
      else {d2_peekvalid=0xffffffff;return 0;}
    }else if(mmu_confused){
...
//      }
      ...
      if(ad>=0xfc0000){
        if(tos_high && ad<(0xfc0000+tos_len)){ROM_PEEK(ad-rom_addr)=val;return true;}
        else return false;
      }
    }else if(ad>=rom_addr  && ad<(0xe00000+tos_len)){
